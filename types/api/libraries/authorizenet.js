...
exports.createAnAcceptPaymentTransaction = (config) => {
    return new Promise((resolve, reject) => {
        ...
        billTo.setAddress(config.billingAddress.street1 +
            (config.billingAddress.street2
                ? " " + config.billingAddress.street2
                : ""));
        ...
        config.billingAddress.phone
            ? billTo.setPhoneNumber(config.billingAddress.phone)
            : null;
        const shipTo = new ApiContracts.CustomerAddressType();
        shipTo.setFirstName(config.shippingAddress.name);
        shipTo.setAddress(config.shippingAddress.street1 +
            (config.shippingAddress.street2
                ? " " + config.shippingAddress.street2
                : ""));
            ...
            if (response != null) {
                if (response.getMessages().getResultCode() ==
                    ...
                    if (response.getTransactionResponse().getMessages() != null) {
                        console.log("Successfully created transaction with Transaction ID: " +
