...
exports.createAnAcceptPaymentTransaction = (config) => {
    return new Promise((resolve, reject) => {
        ...
        billTo.setAddress(config.billingAddress.street1 + (config.billingAddress.street2 ? ' ' + config.billingAddress.street2 : ''));
        ...
        shipTo.setAddress(config.shippingAddress.street1 + (config.shippingAddress.street2 ? ' ' + config.shippingAddress.street2 : ''));
            ...
            if (response != null) {
                if (response.getMessages().getResultCode() == ApiContracts.MessageTypeEnum.OK) {
                    if (response.getTransactionResponse().getMessages() != null) {
                        Sentry.captureException('Successfully created transaction with Transaction ID: ' + response.getTransactionResponse().getTransId());
                        Sentry.captureException('Response Code: ' + response.getTransactionResponse().getResponseCode());
                        Sentry.captureException('Message Code: ' +
                            response
...
// };
// test();
//# sourceMappingURL=authorizenet.js.map